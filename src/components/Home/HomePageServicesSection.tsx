...
const HomePageServicesSection: React.FC = () => {
  ...
  const splitOverlayToChars = (sectionEl: HTMLElement | null) => {
    ...
    const overlayLines = sectionEl.querySelectorAll(".hpss-overlay-line");
    overlayLines.forEach((line) => {
      ...
      for (let i = 0; i < text.length; i++) {
        ...
        span.classList.add("hpss-overlay-char");
        span.setAttribute("aria-hidden", "true");

        if (ch === " ") {
          span.textContent = "\u00A0";
          span.classList.add("hpss-space-char");
        } else {
          span.textContent = ch;
        }
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const galleryItems = gsap.utils.toArray(
        sectionEl.querySelectorAll(".hpss-gallery-item")
      ) as HTMLElement[];

      const overlayChars = sectionEl.querySelectorAll(".hpss-overlay-char");
      ...
      if (slidesSectionEl) {
        const slides = gsap.utils.toArray(
          slidesSectionEl.querySelectorAll(".hpss-slide")
        ) as HTMLElement[];
        ...
        const slidesTimeline = gsap.timeline({
          scrollTrigger: {
            ...
            invalidateOnRefresh: true,
            onLeave: () => {
              // Unpin the section to allow normal scrolling to continue
              ScrollTrigger.getById("hpss-slides-trigger")?.disable();
            },
          },
        });
  ...
  return (
    <div ref={rootRef}>
      ...
      <section
        ref={sectionRef}
        className="hpss-gallery-section"
        style={{ backgroundImage: `url(${backgroundImage})` }}
      >
        <div className="hpss-bg-media">
          <video
            className="hpss-bg-video"
            src={backgroundVideo}
            autoPlay
            muted
            loop
            ...
            aria-hidden="true"
          />
          <div className="hpss-bg-overlay" aria-hidden="true" />
        </div>

        <div className="hpss-section-container">
          {/* Overlay Text */}
          <div
            ref={overlayTextRef}
            className="hpss-section-overlay-text"
            aria-label="Live Better Feel More"
          >
            <div className="hpss-overlay-line" data-text="Live  Better"></div>
            <div className="hpss-overlay-line" data-text="Feel  More"></div>
          </div>

          {/* Gallery */}
          <div
            ref={galleryRef}
            className="hpss-gallery"
            style={{ visibility: "hidden" }}
          >
            <div className="hpss-gallery-item">
      ...
      <section
        ...
        className="hpss-slides-section"
        style={{ backgroundImage: `url(${backgroundImage})` }}
      >
        <div className="hpss-slides-bg-text">
          <div className="hpss-bg-text-line">Creating</div>
          <div className="hpss-bg-text-line">Spaces</div>
          <div className="hpss-bg-text-line">That</div>
          <div className="hpss-bg-text-line">Inspire</div>
        </div>

        <div className="hpss-slides-overlay" />

        <div className="hpss-slide">
          <div className="hpss-slide-image">
            <img src={`${publicUrl}images/l11.jpg`} alt="Modern Architecture" />
          </div>
          <div className="hpss-slide-content">
